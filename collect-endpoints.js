...
function runAudit() {
  ...
  console.log(`💻 Found ${frontend.length} Unique Frontend API Calls in '${path.basename(frontendDir)}'\n`);
    ...
    const beMatch = backend.find(
      (b) =>
        ...
        (b.path === fe.normalizedPath ||
          b.path.replace(/:[a-zA-Z0-9_]+/g, ':id') === fe.normalizedPath.replace(/:[a-zA-Z0-9_]+/g, ':id'))
    );
    ...
    const isCalled = frontend.some(
      (fe) =>
        ...
        (fe.normalizedPath === be.path ||
          fe.normalizedPath.replace(/:[a-zA-Z0-9_]+/g, ':id') === be.path.replace(/:[a-zA-Z0-9_]+/g, ':id'))
    );

    if (!isCalled) {
      console.log(`🔹 [${be.method.padEnd(6)}] ${be.path.padEnd(52)} ➔ ${be.controller}#${be.handler} (${be.roles.join(', ')})`);
    }
  }
